...
try {
  ...
  const open = async()=>{ await p.goto(`${BASE}/repair-order/${roId}`); await p.waitForSelector('[data-testid^="advisor-edit-pricing-"]', {timeout: 60000}); await p.waitForTimeout(1500); await p.evaluate(()=>document.fonts.ready);
    await p.evaluate(()=>{ document.querySelectorAll('button').forEach(b=>{ const cs=getComputedStyle(b); if(cs.position==='fixed' && b.getBoundingClientRect().bottom>window.innerHeight-120) b.style.display='none'; }); }); };
  ...
  const lockedOverride = async(opts)=>{ await p.evaluate(({BUBBLE, badgeCount})=>{ const mkBubble=eval(BUBBLE);
      ...
      card.querySelectorAll('ul.divide-y > li').forEach(li=>{ const row=li.querySelector('.flex.items-center.gap-2'); if(!row) return; const head=row.firstElementChild; head.style.flexWrap='nowrap'; head.style.minWidth='0'; head.style.gap='8px';
        const name=head.querySelector('.font-semibold'); if(name){ name.style.cssText+='min-width:0;flex:1 1 auto;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;'; name.setAttribute('data-pb','name'); }
        const num=head.querySelector('.font-mono'); if(num){ num.style.cssText+='flex:0 1 auto;min-width:0;max-width:7.5rem;overflow:hidden;white-space:nowrap;'; const txt=[...num.childNodes].find(n=>n.nodeType===3); if(txt){ const s=document.createElement('span'); s.textContent=txt.textContent; s.style.cssText='min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;'; s.setAttribute('data-pb','num'); num.replaceChild(s,txt);} }
