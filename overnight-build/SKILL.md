...
### Step 1 — Preflight
- **Sweep the grooming board first** (groom skill SOP 1 — account check, then `read_db` the `rulings` and `notes`
  collections of the board named in BACKLOG.md's header): file every ruling ⛔-verbatim into BACKLOG.md (an item ruled
  on the board may be Queued-ready without ever being ruled in chat), land it on `origin/main`, then delete the swept
  docs. A staged note is a QUESTION — it holds its item back from the run until answered in the frames; a held item
  ("let's talk", `HOLD` in `decisions.py`) is never taken by the run.
